#include "ChatBubble.h"
#include <QPixmap>
#include <QImage>
#include <QLabel>
#include <QPainter>

// 计算空白宽度
#define SpaceWidth() (m_xOffset + m_profileRect.width() + 5 * m_textMargin)

// 默认图片
unsigned char pix_default_head[] = {  };

ChatBubble::ChatBubble(QWidget* parent)
	: ChatBubble(QPixmap(), "xxxxxxxxxxxxxx")  // 委托构造函数, 只允许构造派生类  
{

}

ChatBubble::ChatBubble(const QPixmap& head_img, const QString& message, ChatBubble::BubbleType type, QWidget* parent)
	: QLabel(parent)
	, m_type(type)
	, m_message(message)
	, m_head_img(head_img)
{
	initUi();

	// 头像有一个默认，如果没有传递图片
	if (m_head_img.isNull()) {
		auto img = QImage::fromData(pix_default_head, 1506, "PNG");
		m_head_img = QPixmap::fromImage(img);
	}

	// 设置文字
	QLabel::setText(message);
}

void ChatBubble::setMessage(const QPixmap& head_img, const QString& message, ChatBubble::BubbleType type)
{
	m_type = type;
	m_message = message;
	m_head_img = head_img;

	QLabel::setText(message);
}

void ChatBubble::initUi()
{
	// 大小 这里设置为 (200,70)
	resize(200, 70);
	setAlignment(Qt::AlignTop);   // 设置文本顶部对齐
	setWordWrap(true);   // 自动换行
	setTextInteractionFlags(Qt::TextSelectableByKeyboard);  // 允许鼠标点中
	QLabel::setFont(QFont("楷体", 18));   // 设置字体

	// 设置头像大小
	m_profileRect.setSize(QSize(42, 42));
}

void ChatBubble::paintEvent(QPaintEvent* ev)
{
	// 创建画笔
	QPainter painter(this);
	painter.setRenderHint(QPainter::RenderHint::Antialiasing);  // 抗锯齿
	painter.setPen(Qt::NoPen);
	painter.setBrush(QColor(60, 172, 213));

	// 绘制头像
	painter.drawPixmap(m_profileRect, m_head_img);

	// 绘制气泡图
	painter.drawRoundedRect(m_bubbleRect, 10, 10);

	//绘制三角形
	painter.drawPolygon(m_points);

	QLabel::paintEvent(ev);
}

// 从新设置控件大小
void ChatBubble::resizeEvent(QResizeEvent* ev)
{
	// 气泡图、文本大小
	updateBubbleSize();
	updateTextRect();

	// 获取包含当前 QListWidgetItem 的 QListWidget，并在确认该 QListWidget 存在的情况下设置该项的大小提示（sizeHint）。
	auto listw = QListWidgetItem::listWidget();
	if (listw)
	{
		QListWidgetItem::setSizeHint(QSize(1, size().height()));
	}
}

void ChatBubble::updateBubbleSize()
{
	// 左边
	if (m_type == ChatBubble::BubbleLeft) {
		// 绘制头像大小
		m_profileRect.moveTopLeft(QPoint(m_xOffset, m_yOffset));

		// 计算冒泡图矩阵位置， 先计算文本矩阵大小
		// 冒泡图大小 文本大小决定
		m_bubbleRect = m_textRect.adjusted(-m_textMargin, -m_textMargin, m_textMargin, m_textMargin);

		m_points.clear();
		m_points.append(QPointF(m_bubbleRect.x(), m_bubbleRect.y() + 21 - 6));
		m_points.append(QPointF(m_bubbleRect.x(), m_bubbleRect.y() + 21 + 12 - 6));
		m_points.append(QPointF(m_bubbleRect.x() - 6, m_bubbleRect.y() + 21));
	}
	else if (m_type == ChatBubble::BubbleRight) {

		m_profileRect.moveTopRight(QPoint(width() - m_xOffset, m_yOffset));

		// 右边一样
		m_bubbleRect = m_textRect.adjusted(-m_textMargin, -m_textMargin, m_textMargin, m_textMargin);

		m_points.clear();
		m_points.append(QPointF(m_bubbleRect.right(), m_bubbleRect.y() + 21 - 6));
		m_points.append(QPointF(m_bubbleRect.right(), m_bubbleRect.y() + 21 + 12 - 6));
		m_points.append(QPointF(m_bubbleRect.right() + 6, m_bubbleRect.y() + 21));
	}

	setFixedHeight(m_bubbleRect.height() + 1 * m_textMargin);

	// 设置内容边距
	setContentsMargins(
		m_textRect.x(),
		m_textRect.y(),
		width() - (m_textRect.right() + 1),
		height() - (m_textRect.bottom() + 1)
	);

}

void ChatBubble::updateTextRect()
{
	// 获得文本尺寸
	auto fm = this->fontMetrics();
	// 计算文本边距
	auto bRect = fm.boundingRect(QRect(), Qt::AlignLeft, m_message);

	if (m_type == ChatBubble::BubbleLeft) {
		// 文本边框
		/*
		* x: 偏移量 + 头像大小 + 边距 * 3
		* y: 偏移量 + 文本边框宽度
		*/
		m_textRect.moveTopLeft(QPoint(
			m_xOffset + m_profileRect.width() + 3 * m_textMargin,
			m_yOffset + m_textMargin
		));

		// 调整文本宽度: 空闲位置不够
		if (bRect.width() <= width() - SpaceWidth()) {
			m_textRect.setSize(bRect.size());
		}
		else {
			// 空闲位置够，设置宽、高
			m_textRect.setWidth(width() - SpaceWidth());
			// 高看多少行
			m_textRect.setHeight(realLineNumber() * fm.lineSpacing() / 2);   // 垂直矩阵
		}
	}
	else if (m_type == ChatBubble::BubbleRight) {
		// 字体不够放得下
		if (bRect.width() <= width() - SpaceWidth())
		{
			// 设置起点，这个就像是微信回消息的位置
			m_textRect.setX(width() - bRect.width() - m_profileRect.width() - 3 * m_textMargin);
			m_textRect.setY(m_yOffset + m_textMargin);;
			m_textRect.setSize(bRect.size());
		}
		else
		{
			// 字体放不下，需要计算字体行数
			m_textRect.setX(m_xOffset + 2 * m_textMargin);
			m_textRect.setY(m_textMargin);

			m_textRect.setWidth(width() - SpaceWidth());
			m_textRect.setHeight(realLineNumber() * fm.lineSpacing() + fm.lineSpacing() / 2);
		}
	}
}

std::vector<int> ChatBubble::textWidthList() const
{
	std::vector<int> vec;
	auto fm = this->fontMetrics();

	qsizetype linePos = 0;  // \n的位置
	QString str = m_message;
	for (int i = 0; i < str.length(); i = linePos + 1) {
		linePos = str.indexOf("\n", i);	 //检查是不是 \n
		if (linePos == -1) {  // 最后不是 \n, 找不到 \n, 多加1行
			// 子宽度
			int curLineW = fm.boundingRect(QRect(), Qt::AlignLeft, str.mid(i)).width();
			vec.push_back(curLineW);
			break;
		}
		else {
			// 核心：计算长度
			int curLineW = fm.boundingRect(QRect(), Qt::AlignLeft, str.mid(i, linePos - i)).width();
			vec.push_back(curLineW);
		}
	}

	return vec;
}

int ChatBubble::lineNumber() const
{
	return textWidthList().size();
}

int ChatBubble::realLineNumber() const
{
	auto vec = textWidthList();
	int nLine = vec.size();

	for (auto w : vec) {
		auto num = w / m_textRect.width();
		if (num == 0) {

		}
		else {  // 一行太长，容不下了，换行
			nLine += num;
		}
	}

	return nLine;
}
